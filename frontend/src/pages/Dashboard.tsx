...
export function Dashboard() {
  ...
  const quota = useQuotaStore((state) => state.mine);
  const quotaLoading = useQuotaStore((state) => state.loading);
  const loadMineQuota = useQuotaStore((state) => state.loadMine);
  const { token } = useAuth();
  ...
  useEffect(() => {
    ...
    void loadGoals();
    void loadMineQuota();
  }, [loadActivities, loadGoals, loadMineQuota, token]);

  return (
    <Space direction="vertical" size={20} style={{ width: '100%' }}>
      <div>
        <Typography.Title level={2}>CarbonTrack 工作台</Typography.Title>
        <Typography.Text type="secondary">今日、周期、目标进度和地区额度集中在一个视图中。</Typography.Text>
      </div>
      <Row gutter={[16, 16]}>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={9}>
          <Space direction="vertical" size={12} style={{ width: '100%' }}>
            <QuotaStatusCard quota={quota} loading={quotaLoading} />
            <Card title="目标进度">
              <Space direction="vertical" size={12} style={{ width: '100%' }}>
